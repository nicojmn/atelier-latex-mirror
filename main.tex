...
\begin{frame}[fragile]{Les paragraphes avec \LaTeX{}}
	...
	\item Ajouter de l'espace entre les paragraphes et changer l'indentation
	  ...
	  \begin{lstlisting}[style=nonumbers]
	  \setlength{\parindent}{15pt} % Remet l'indentation par défault
	  \end{lstlisting}
...
\begin{center}
  \begin{tabular}{lll}
    \lstinline|$x + 1 = 2$| & $\Rightarrow$ &  $x + 1 = 2$\\
    \medskip
    \lstinline|$\frac{1}{x}$| & $\Rightarrow$ &  $\frac{1}{x}$\\
  \end{tabular}
\end{center}
	\item Les opérateurs, symboles,\dots commencent par \textbf{\textbackslash}, sauf \lstinline|+, -, /, ^, _,...|
	\begin{center}
		\begin{tabular}{lllr}
			\lstinline|$a^{11}$| & $\Rightarrow$ & $a^{11}$ & Good \\
			\lstinline|$a^11$| & $\Rightarrow$ & $a^11$ & Bad ! \\
			\lstinline|$\sin(x)$| & $\Rightarrow$ & $\sin(x)$ & Good \\
			\lstinline|$sin(x)$| & $\Rightarrow$ & $sin(x)$ & Bad !\\
			\lstinline|$\frac{\Theta}{\sqrt{\beta}}$| & $\Rightarrow$ & $\frac{\Theta}{\sqrt{\beta}}$ & 
...
\section{Les maths et les polices}
\begin{frame}[fragile]{Les maths et les polices}
\begin{itemize}
	\item Parfois, certaines variables sont composées de plusieurs lettres. On doit utiliser des polices différentes comme \lstinline|\mathrm| ou \lstinline|\mathsf|
	\begin{center}
			\begin{tabular}{llll}
				\lstinline|$Var(x)$| & $\Rightarrow$ & $Var(x)$ & Bad ! \\
				\lstinline|$\mathrm{Var}(x)$| & $\Rightarrow$ & $\mathrm{Var}(x)$ & Good 
			\end{tabular}
	\end{center}
\end{itemize}
