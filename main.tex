...
Introduction à l'écriture de documents \textrm{\LaTeX{}}}
...
\begin{frame}
  ...
  à David \textsc{Ernst} et Matthieu \textsc{Baerts} pour avoir réalisé la deuxième version, ainsi qu'à Arnaud \textsc{Cerckel} et Benoît \textsc{Legat} pour avoir réalisé la troisième version.
\end{frame}
...
\begin{frame}{Pourquoi \LaTeX{}?}
  ...
  \begin{itemize}
  	\item Qualité professionnelle de documents
...
\begin{frame}{Oui mais...}
  \begin{center}
    %\resizebox{\textwidth}{!}{
    \begin{tikzpicture}
      \begin{axis}[ticks=none,xmin=0, xmax=4, ymin=-2, ymax=5,
...
\item \textbf{Commande} 
	...
	\item S'applique à une partie du texte, délimitée par des accolades
...
\begin{tabular}{lp{8cm}}
  ...
  \textbf{report} & pour de plus longs rapports de plusieurs chapitres, petits livres, thèses,\dots \\
...
\begin{frame}[fragile]{Les paragraphes avec \LaTeX{}}
	...
	\item Ajouter de l'espace entre les paragraphes et changer l'indentation
	  \begin{lstlisting}[style=nonumbers]
	  \usepackage{parskip} % Ajoute de l'espace entre les paragraphes et met l'indentation à 0
	  \end{lstlisting}
	  \includegraphics[scale=0.35]{par1.png}
	  \begin{lstlisting}[style=nonumbers]
	  \setlength{\parindent}{15pt} % Remet l'indentation par défaut
	  \end{lstlisting}
	  \includegraphics[scale=0.35]{par2.png}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
  	\item Par défaut, c'est justifié.
  \begin{lstlisting}[style=nonumbers]
...
\subsection{Listes}
\begin{frame}[fragile]
  \frametitle{\texttt{itemize} et \texttt{enumerate}}
  \begin{columns}
...
\end{tabular}
  	...
  	\begin{itemize}
  		...
  		\item Le contenu des <lignes> est séparé en colonnes par \lstinline|&|
...
\begin{frame}[fragile]{Troisième exercice}
  ...
  \begin{itemize}
	  ...
	  \begin{itemize}
	  	\item écrire un peu de \textbf{texte};
        \item ajouter une \textbf{figure} (flottant) avec une \textbf{légende} (\texttt{caption}) et \textbf{référence} (\texttt{label});
	  ...
	  \begin{itemize}
        ...
        \item ajouter un \textbf{tableau} (flottant) avec une \textbf{légende} (\texttt{caption}) et \textbf{référence} (\texttt{label});
	  \end{itemize}
  \end{itemize}
\end{frame}
...
\subsection{\texttt{footnote}}
\begin{frame}[fragile]
  \frametitle{\texttt{footnote}}
  \begin{lstlisting}
...
\subsection{\texttt{include} et \texttt{input}}
\begin{frame}[fragile]
  \frametitle{\texttt{include} et \texttt{input}}
  \begin{columns}
    \begin{column}{0.5\textwidth}
...
\end{center}
	...
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\item Les packages \lstinline|amsmath| et \lstinline|amssymb| apportent beaucoup d'environnements et symboles supplémentaires très utiles, à inclure par défaut.
\end{itemize}
\end{frame}
...
\begin{itemize}
\item On peut aussi ajouter une formule mathématique centrée hors du texte entre deux symboles \textbf{\$\$} ou entre \lstinline|\[ ... \]|.
\vspace{0.5cm}
\begin{columns}
  \begin{column}{0.45\textwidth}
...
\begin{frame}[fragile]{Matrices}
	\begin{itemize}
		\item Les matrices s'écrivent avec l'environnement \lstinline|array| (fonctionnement semblable à \lstinline|tabular|).
			\begin{columns}
				\begin{column}{0.45\textwidth}
					$$\left(
					\begin{array}{c c}
